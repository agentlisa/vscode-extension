...
import { ScanResult, ScanResultIssue } from '../models';

export class ScanResultsProvider implements vscode.TreeDataProvider<ScanResultItem> {
    private _onDidChangeTreeData: vscode.EventEmitter<ScanResultItem | undefined | null | void> = new vscode.EventEmitter<ScanResultItem | undefined | null | void>();
    readonly onDidChangeTreeData: vscode.Event<ScanResultItem | undefined | null | void> = this._onDidChangeTreeData.event;
    ...
    getChildren(element?: ScanResultItem): Thenable<ScanResultItem[]> {
        ...
        if (element.contextValue === 'issues' && element.scanResult) {
            return Promise.resolve(this.getIssueItems(element.scanResult.result, element.scanResult));
        }
        
        if (element.contextValue === 'issue' && element.issue) {
            return Promise.resolve(this.getAffectedFileItems(element.issue.affectedFiles));
        }
    ...
    private getScanIssues(scanResult: ScanResult): ScanResultItem[] {
        ...
        if (scanResult.status === 'pending' || scanResult.status === 'processing') {
            ...
            if (durationMinutes > 0) {
                statusText += ` (running for ${durationMinutes} minute${durationMinutes > 1 ? 's' : ''})`;
            } else {
                statusText += ' (just started)';
            }
        }
        
        items.push(new ScanResultItem(
            statusText,
        ...
        } else if (scanResult.status === 'pending' || scanResult.status === 'processing') {
            items.push(new ScanResultItem(
        ...
        if (items.length === 2) { // Changed from 1 to 2 since we always have status + open website
            items.push(new ScanResultItem(
    ...
    private getIssueItems(issues: ScanResultIssue[], scanResult: ScanResult): ScanResultItem[] {
        return issues.map(issue => new ScanResultItem(
            `${issue.severity}: ${issue.title}`,
            issue.affectedFiles.length > 1 ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None,
            'issue',
    ...
    private getAffectedFileItems(affectedFiles: ScanResultIssue['affectedFiles']): ScanResultItem[] {
        return affectedFiles.map(file => new ScanResultItem(
            `${path.basename(file.filePath)} (Line ${file.range.start.line})`,
            vscode.TreeItemCollapsibleState.None,
            'affectedFile',
            undefined,
            file.filePath,
    ...
    public async openAffectedFile(affectedFile: ScanResultIssue['affectedFiles'][0]): Promise<void> {
        await this.openFileAtRange(affectedFile.filePath, affectedFile.range);
    }
    ...
    private async openFileAtRange(filePath: string, range: { start: { line: number, column: number }, end: { line: number, column: number } }): Promise<void> {
        try {
            ...
            editor.revealRange(rangeToSelect, vscode.TextEditorRevealType.InCenterIfOutsideViewport);
    ...
    public async viewAllIssues(scanResult: ScanResult, focusIssueId?: string): Promise<void> {
            ...
            panel.webview.onDidReceiveMessage(async (message) => {
                try {
                    ...
                    vscode.window.showErrorMessage(
                        `Failed to handle file operation: ${error instanceof Error ? error.message : String(error)}`
                    );
                }
            });
    ...
    public async viewIssueDetails(issue: ScanResultIssue, scanResult?: ScanResult): Promise<void> {
    ...
    public async viewCodeSummary(scanResult: ScanResult): Promise<void> {
        if (!scanResult.codeSummary) {
            vscode.window.showInformationMessage('No code summary available for this scan.');
            return;
        }
    ...
    private getIssueDetailsHtml(issue: ScanResultIssue): string {
        ...
        const affectedFilesHtml = issue.affectedFiles.map(file => `
            <div class="affected-file">
                <strong>File:</strong> ${file.filePath}<br>
                <strong>Range:</strong> Line ${file.range.start.line}, Column ${file.range.start.column} 
                → Line ${file.range.end.line}, Column ${file.range.end.column}
            </div>
        `).join('');
        ...
        return `
            ...
            <body>
                <h1>${issue.title}</h1>
                <div class="severity" style="background-color: ${severityColor[issue.severity] || '#888'}">${issue.severity}</div>
    ...
    private getScanResultHtml(scanResult: ScanResult): string {
        return `
            ...
            <body>
                ...
                <div class="status ${scanResult.status}">${scanResult.status}</div>
                
                <div class="info">
                    <strong>Scan ID:</strong> ${scanResult.id}
                    <br>
                    <strong>Created:</strong> ${new Date(scanResult.createdAt).toLocaleString()}
                    <br>
                    ${scanResult.completedAt ? `<strong>Completed:</strong> ${new Date(scanResult.completedAt).toLocaleString()}<br>` : ''}
                ...
                ${scanResult.result ? `
                    <h2>Results</h2>
                    <div class="result-content">${JSON.stringify(scanResult.result, null, 2)}</div>
                ` : ''}
                
                ${scanResult.metadata ? `
                    <h2>Metadata</h2>
                    <div class="result-content">${JSON.stringify(scanResult.metadata, null, 2)}</div>
                ` : ''}
            </body>
            </html>
        `;
    ...
    private getAllIssuesHtml(scanResult: ScanResult, focusIssueId?: string): string {
        if (!scanResult.result || scanResult.result.length === 0) {
            return `
                <!DOCTYPE html>
                <html>
        ...
        const severityOrder = ['CRITICAL', 'HIGH', 'MEDIUM', 'LOW', 'WARN', 'INFORMATIONAL'] as const;
        ...
        const severityGroupsHtml = severityOrder
            ...
            .map(severity => {
                ...
                const issuesHtml = issues.map(issue => {
                    const affectedFilesHtml = issue.affectedFiles.map((file, index) => `
                        <div class="affected-file clickable-file" onclick="openFile('${file.filePath}', ${file.range.start.line}, ${file.range.start.column}, ${file.range.end.line}, ${file.range.end.column}, this)">
                            <strong>File:</strong> ${file.filePath}<br>
                            <strong>Location:</strong> Line ${file.range.start.line}, Column ${file.range.start.column} 
                            → Line ${file.range.end.line}, Column ${file.range.end.column}
                        </div>
                    `).join('');

                    return `
                        <div class="issue-item ${focusIssueId === issue.id ? 'focused' : ''}" id="issue-${issue.id}">
                            <div class="issue-header" onclick="toggleIssue('${issue.id}')">
                                <span class="toggle-icon" id="toggle-${issue.id}">▶</span>
                                <span class="issue-title">${issue.title}</span>
                            </div>
                            <div class="issue-content" id="content-${issue.id}" style="display: none;">
                                ${issue.description ? `
                                    <div class="section">
                                        <h4>Description</h4>
                                        <div class="markdown-content">${this.markdownToHtml(issue.description)}</div>
                                    </div>
                                ` : ''}
                                
                                <div class="section">
                                    <h4>Affected Files (${issue.affectedFiles.length})</h4>
                                    ${affectedFilesHtml}
                                </div>
                                ...
                                ${issue.recommendation ? `
                                    <div class="section">
                                        ...
                                        <div class="markdown-content">${this.markdownToHtml(issue.recommendation)}</div>
                                    </div>
                                ` : ''}
                            </div>
                        </div>
                ...
                return `
                    <div class="severity-group">
                        <div class="severity-header" onclick="toggleSeverity('${severity}')">
                            ...
                            <span class="severity-badge" style="background-color: ${severityColors[severity]}">${severity}</span>
        ...
        return `
            ...
            <body>
                ...
                <div class="scan-header">
                    ...
                    <strong>Issues Found:</strong> ${scanResult.result.length}<br>
                    <strong>Completed:</strong> ${scanResult.completedAt ? new Date(scanResult.completedAt).toLocaleString() : 'In progress'}
                </div>
                ...
                <script>
                    ...
                    document.addEventListener('DOMContentLoaded', function() {
                        ${focusIssueId ? `expandToIssue('${focusIssueId}');` : ''}
                    });
                </script>
            </body>
            </html>
        ...
        return `
            ...
            <body>
                ...
                <div class="scan-info">
                    ...
                    <strong>Created:</strong> ${new Date(scanResult.createdAt).toLocaleString()}
                    ${scanResult.completedAt ? `<br><strong>Completed:</strong> ${new Date(scanResult.completedAt).toLocaleString()}` : ''}
                </div>
...
export class ScanResultItem extends vscode.TreeItem {
    ...
    private getTooltip(): string {
        if (this.contextValue === 'scanResult' && this.scanResult) {
            ...
            return `Status: ${status}\nCreated: ${new Date(this.scanResult.createdAt).toLocaleString()}`;
        }
        
        if (this.contextValue === 'issues' && this.scanResult) {
            return `Click to view all ${this.scanResult.result.length} issue${this.scanResult.result.length > 1 ? 's' : ''} in a unified panel`;
        }
    ...
    private getIcon(): vscode.ThemeIcon {
        if (this.contextValue === 'scanResult' && this.scanResult) {
            switch (this.scanResult.status) {
                ...
                case 'cancelled':
                    return new vscode.ThemeIcon('stop', new vscode.ThemeColor('testing.iconSkipped'));
                default:
